import { AnyType, ArrayType, BoolType, EntityType, FunType, LooseRecordType, NoneType, NumberType, OrType, PointerType, StringType, TupleType, Type, TypeType } from "../shared/types.ts";
        ...
        case AstType.Binary: {
            ...
            if (a.type == AstType.Number && b.type == AstType.Number) {
                switch ((n as BinaryNode).op) {
                    case '+':
                        return { type: AstType.Number, n: (a as NumberNode).n+(b as NumberNode).n } as NumberNode;
                    case '-':
                        return { type: AstType.Number, n: (a as NumberNode).n-(b as NumberNode).n } as NumberNode;
                    case '*':
                        return { type: AstType.Number, n: (a as NumberNode).n*(b as NumberNode).n } as NumberNode;
...
export class Parser {
    ...
    private throw(msg: string): never {
        throw `${msg} (line ${this.current.ln})`
    }
    ...
    private orType(): Type {
        ...
        if (this.current.type == TokenType.Pipe) {
            this.adv();
            return { type: TypeType.Or, a, b: this.refType() } as OrType;
        }
        return a;
    }
    ...
    private funDecl(name: string|undefined): Ast {
        ...
        return { type: AstType.FunDecl, name, params, body, ret } as FunDeclNode;
    }
    ...
    private record(name: string | undefined): RecordNode {
        ...
        this.expect(TokenType.RightBrace);
        return { type: AstType.Record, body, name } as RecordNode;
    }
}